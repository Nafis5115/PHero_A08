...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow px-6 md:px-12 py-3 flex items-center justify-between relative">
        ...
        {links.map((link) => {
          ...
          return (
            <li key={link.name} className="relative">
              ...
              <span
                className={`absolute left-0 -bottom-0.5 h-[2px] bg-gradient-to-r from-[#632EE3] to-[#9F62F2] transition-all duration-500 ease-in-out ${
      ...
      <a
        href="https://github.com/nafis5115"
        className="hidden md:flex items-center cursor-pointer gap-2 px-4 py-2 bg-gradient-to-r from-[#632EE3] to-[#9F62F2] text-white rounded-md hover:opacity-90 transition-all"
      >
        <svg
          className="h-6"
      ...
      >
        {isOpen ? (
          <X className="h-7 w-7 text-[#001931]" />
        ) : (
          <Menu className="h-7 w-7 text-[#001931]" />
        )}
      </button>

      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-white shadow-md rounded-b-md flex flex-col items-center py-4 z-50">
          {links.map((link) => {
            const isActive = location.pathname === link.href;
            return (
              <Link
                ...
                className={`py-2 text-lg transition-all duration-300 ${
                  isActive
          ...
          <a
            href="https://github.com/nafis5115"
            className="flex items-center cursor-pointer gap-2 px-4 py-2 bg-gradient-to-r from-[#632EE3] to-[#9F62F2] text-white rounded-md hover:opacity-90 transition-all"
          >
            <svg
              className="h-6"
              viewBox="0 -3.5 256 256"
